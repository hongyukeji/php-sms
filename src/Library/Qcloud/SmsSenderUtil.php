...
namespace Hongyukeji\PhpSms\Library\Qcloud;
...
class SmsSenderUtil
{
     ...
     * @param array  $mobile_number  手机号码
     * @return string  签名结果
     */
    public function calculateSig($appkey, $random, $curTime, $mobile_number)
    {
        $mobile_numberString = $mobile_number[0];
        for ($i = 1; $i < count($mobile_number); $i++) {
            $mobile_numberString .= ("," . $mobile_number[$i]);
        }

        return hash("sha256", "appkey=".$appkey."&random=".$random
            ."&time=".$curTime."&mobile=".$mobile_numberString);
    }
    ...
    public function phoneNumbersToArray($nationCode, $mobile_number)
    {
        $i = 0;
        $tel = array();
        do {
            ...
            $telElement->mobile = $mobile_number[$i];
            array_push($tel, $telElement);
        } while (++$i < count($mobile_number));
    ...
    {
        $mobile_number = array($phoneNumber);
